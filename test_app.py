...
def test_subtract(self):
    ...
    response = self.client.post("/math/subtract", json=payload)
    data = response.get_json()
    self.assertEqual(data["result"], 6)

def test_multiply(self):
    payload = {"num1": 6, "num2": 7}
    response = self.client.post("/math/multiply", json=payload)
    data = response.get_json()
    self.assertEqual(data["result"], 42)

def test_divide(self):
    payload = {"num1": 8, "num2": 2}
    response = self.client.post("/math/divide", json=payload)
    data = response.get_json()
